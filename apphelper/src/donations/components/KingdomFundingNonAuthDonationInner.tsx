...
// entered in Collect.js's hosted fields and never touch our backend), so it's enabled.
...
export const KingdomFundingNonAuthDonationInner: React.FC<Props> = ({ mainContainerCssProps, showHeader = true, ...props }) => {
  ...
  // Card vs. bank (ACH). Both are tokenized via NMI Collect.js — the bank account
  // number / routing number are entered in Collect.js hosted fields, not here.
  const [payMethod, setPayMethod] = useState<"card" | "ach">(
    KF_ACH_ENABLED && props.paymentType === "bank" ? "ach" : "card"
  );
  ...
  const validate = () => {
    ...
    // Card/bank field validation is handled inside the Collect.js hosted form.
    setErrors(result);
  ...
  const processDonation = async (person: any) => {
    ...
    try {
      // Both card and ACH tokenize through NMI Collect.js, which returns a single-use
      // payment_token. The token carries the payment instrument, so no raw card/bank
      // numbers are sent to our backend.
      ...
      basePayload.type = paymentType; // "card" or "bank"
  ...
  return (
    ...
    >
      ...
      {/* Payment input — card or bank (ACH), both tokenized via NMI Collect.js */}
      {gateway?.publicKey ? (
